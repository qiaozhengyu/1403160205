...
void CenterFrame::createUserCommandArea()
{
    ...
    QPointF pt7(-3+p.size().width(),p.size().height()/2);
    QVector<QPointF> ptb;
    ...
    //图片
    btndrawpicture=new QPushButton;
    btndrawpicture->setToolTip("绘图片");
    btndrawpicture->setCheckable(true);
    btndrawpicture->setIconSize(p.size());
    ...
    btndrawpicture->setIcon (QIcon(p));
    connect(btndrawpicture,&QPushButton::clicked,this, &CenterFrame::on_btnDrawpictureClicked);
    ...
    gridLayout->addWidget(btndrawpicture,3,0);
...
//插入图片响应槽函数
void CenterFrame::on_btnDrawpictureClicked()
{

    if(btndrawpicture->isChecked())
         {
             drawWidget->setShapeType(ST::picture);
             drawWidget->drawpicture();
             updateButtonStatus();
         }
         else
         {
